/*
    ...
    ALL CONTENT OF THIS CODE IS ONLY WORKING FOR LIVING ROOM
*/
...
int ceilingPushButton = D5;
int balconyPushButton = D6;
...
void setup() {
  ...
  pinMode(ceilingPushButton, INPUT_PULLUP);
  pinMode(balconyPushButton, INPUT_PULLUP);
...
void loop() {
  ...
  if (digitalRead(ceilingPushButton) == LOW) {
    // Button is pressed
    ceilingState = !ceilingState;  // Toggle the LED state
  ...
  devices_lights_0["name"] = "Ceiling Light";
  devices_lights_0["is_active"] = ceilingState;

  JsonObject devices_lights_1 = devices_lights.createNestedObject();
  devices_lights_1["name"] = "Balcony Light";
  devices_lights_1["is_active"] = balconyState;
  ...
  if (httpResponseCode > 0) {
    ...
    if (jsonError) {
      ...
      ceilingState = ceilingLightStatus;

      const char *balconyLightName = lightsDoc["lights"][1]["name"];
      bool balconyLightStatus = lightsDoc["lights"][1]["is_active"];
      balconyState = balconyLightStatus;
  ...
  digitalWrite(ceilingLight, ceilingState ? HIGH : LOW); 
  digitalWrite(balconyLight, balconyState ? HIGH : LOW); 
